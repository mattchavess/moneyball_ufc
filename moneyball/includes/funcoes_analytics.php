...
// Conta lutadores por país (usado na lista "Lutadores por País")
...
// Conta lutadores por estilo de luta (usado na lista "Estilo de Luta do Plantel")
function distribuicaoPorEstilo($conexao) {
    $sql = "SELECT estilo_luta, COUNT(*) AS total
            FROM lutadores
            WHERE estilo_luta <> ''
            GROUP BY estilo_luta
            ORDER BY total DESC";
    $resultado = mysqli_query($conexao, $sql);
    return mysqli_fetch_all($resultado, MYSQLI_ASSOC);
}
...
function evolucaoWinRatePorTemporada($conexao) {
    ...
    foreach ($linhas as $linha) {
        $totalLutas = $linha["total_vitorias"] + $linha["total_derrotas"] + $linha["total_empates"];
        $winRate = $totalLutas > 0 ? ($linha["total_vitorias"] / $totalLutas) * 100 : 0;
...
/**
 * CONSISTÊNCIA (documentar isso também, junto com a fórmula do score):
 *
...
function calcularConsistencia($estatisticasDoLutador) {
    ...
    foreach ($estatisticasDoLutador as $temporada) {
        $totalLutas = $temporada["vitorias"] + $temporada["derrotas"] + $temporada["empates"];
    ...
    $media = array_sum($winRatesPorTemporada) / $n;

    $somaDosQuadrados = 0;
    foreach ($winRatesPorTemporada as $wr) {
        $somaDosQuadrados += pow($wr - $media, 2);
    }
    $desvioPadrao = sqrt($somaDosQuadrados / $n);

    $consistencia = 100 - $desvioPadrao;
    return round(max(0, $consistencia), 1); // nunca deixa ficar negativo
}
...
function top5ScoreConsistencia($conexao) {
    ...
    foreach ($top5 as &$lutador) {
        $completo = buscarLutadorComEstatisticas($conexao, $lutador["id"]);
        $lutador["consistencia"] = calcularConsistencia($completo["estatisticas"]);
    }
    unset($lutador); // boa prática após foreach com &
